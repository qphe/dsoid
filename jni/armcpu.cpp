...
u32 armcpu_switchMode(armcpu_t *armcpu, u8 mode)
{
	...
	armcpu->R[16] &= 0xFFFFFFE0;
	armcpu->R[16] |= (mode & 0x1F);
	armcpu->changeCPSR();
	return oldmode;
}
...
//#endif
	int debug_proc_id = ARMCPU_ARM9;

	bool cond_log = armcpu->instruct_adr == 0x205c1f0;
	cond_log |= (yothumb>0);

	if(armcpu->proc_ID == debug_proc_id && go && cond_log)
	{
		for(int i=0;i<15;i++)
		LOGE("r%u %x", i, armcpu->reg[i]);
		//LOGE("r_user 13 %x", NDS_ARM9.R13_usr);
		//LOGE("r_user 14 %x", NDS_ARM9.R14_usr);
		LOGE("pc %x next instruct %x cycles %u", armcpu->instruct_adr, armcpu->instruction, total_cycle_arm9);

		LOGE("CPSR %x", armcpu->CPSR.val);

		go=false;
		gogo=true;
	}

	bool cond_log2 = armcpu->instruct_adr ==  0x20490ac &&armcpu->R[14] == 0x2049358;//armcpu->instruct_adr == adr_want
	cond_log2 |= (yothumb>0);

	if(armcpu->proc_ID == debug_proc_id && gogo && cond_log2)
	{
		//for(int i=0;i<15;i++)
		//	LOGE("r%u %x", i, armcpu->reg[i]);
				//LOGE("r_user 13 %x", NDS_ARM9.R13_usr);
				//LOGE("r_user 14 %x", NDS_ARM9.R14_usr);
		LOGE("pc %x next instruct %x cpsr %x lr %x", armcpu->instruct_adr, armcpu->instruction,  armcpu->CPSR.val, armcpu->R[14]);

		yothumb++;
	}


	bool cond_stop = armcpu->R[12] == 0x665c;

	if(armcpu->proc_ID == debug_proc_id && gogo && cond_stop && yothumb>0)
	{
		gogo=false;
	...
	if(armcpu->proc_ID == debug_proc_id)
		last_instruct=armcpu->instruction;
...
#endif
    	if(armcpu->proc_ID == 0)
    	{  /*LOGE("irq exception triggered");
    LOGE("pc %x", armcpu->R[15]);
    sleep(10);
    exit(0);*/}
	tmp.val = armcpu->R[16];
