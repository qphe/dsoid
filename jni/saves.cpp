...
SFORMAT SF_ARM7[]={
	...
	{ "7REG", 4,16, NDS_ARM7.reg},
	{ "7CPS", 4, 1, &NDS_ARM7.CPSR },
	{ "7DRG", 4,16, NDS_ARM7.dynarec_reg},
...
SFORMAT SF_ARM9[]={
	...
	{ "9REG", 4,16, NDS_ARM9.reg},
	{ "9CPS", 4, 1, &NDS_ARM9.CPSR },
	{ "9DRG", 4,16, NDS_ARM9.dynarec_reg},
...
#ifdef HAVE_DYNAREC
extern void flush_translation_cache_rom();
#endif
...
void loadstate_slot(int num)
{
   ...
   else
   {
	...
	for(int i=0;i<15;i++)
	{
		NDS_ARM9.dynarec_reg[i]=1;
		NDS_ARM7.dynarec_reg[i]=1;
	}

	LOGE("done loading state");
...
bool savestate_load(EMUFILE* is)
{
	...
	if(nds.ConsoleType != CommonSettings.ConsoleType) {
		LOGE("WARNING: forcing console type to: ConsoleType=%d\n",nds.ConsoleType);
	}

	if((nds._DebugConsole!=0) != CommonSettings.DebugConsole) {
			LOGE("WARNING: forcing console debug mode to: debugmode=%s\n",nds._DebugConsole?"TRUE":"FALSE");
	}
