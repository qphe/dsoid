...
inline void end_timing()
{
	...
	clock_gettime(CLOCK_MONOTONIC, &finish_time);
		...
		coproc_time += (finish_time.tv_sec - start_time.tv_sec);
		coproc_time += (finish_time.tv_nsec - start_time.tv_nsec) / 1000000000.0;

		if(coproc_time>sec)
		{
			LOGE("time start %d %d end %d %d", start_time.tv_sec, start_time.tv_nsec, finish_time.tv_sec, finish_time.tv_nsec);
			LOGE("mosaic time %f", coproc_time);
			sec++;
			//sleep(2);
		}
