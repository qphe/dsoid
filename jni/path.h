...
public:
	...
	void LoadModulePath()
	{
...
#else
		JNIEnv* javaEnv;
		gJVM->GetEnv((void**)&javaEnv, JNI_VERSION_1_6);
		//Qingping He
		jmethodID func = javaEnv->GetStaticMethodID(FilePathRef,"LoadModulePath","()Ljava/lang/String;");
		...
		jstring jPath = (jstring)javaEnv->CallStaticObjectMethod(FilePathRef,func,0);
		const char* cwd=javaEnv->GetStringUTFChars(jPath,0);
